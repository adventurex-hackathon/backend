...
def convert_srt_to_ass(input_srt_path, output_ass_path):
    ...
    def replace_expression(match):
        return latex.get()
    modified_file = re.sub(pattern, replace_expression, content)
    if not os.path.exists('./manim/demo/media'):
        os.makedirs('./manim/demo/media')
...
def generate_image_ass(raw_latex, match_counter):
    png_path = "./manim/demo/media/" + str(match_counter) + ".png"
...
def latex_to_transparent_image(latex_code, output_path):
    ...
    text = ax.text(0, 0, f'${latex_code}$', fontsize=14, color='white', va='bottom', ha='left')
    ...
    plt.savefig(output_path, bbox_inches='tight', pad_inches=0.05, transparent=True, dpi=300)
    plt.close()

# replace path

# replace file names as appropriate
convert_srt_to_ass("./manim/demo/media/videos/main/1080p60/Demo.srt", "./manim/demo/media/temp.ass")
process_latex_from_ass("./manim/demo/media/temp.ass", "./manim/demo/media/Demo.ass")
os.remove('./manim/demo/media/temp.ass')